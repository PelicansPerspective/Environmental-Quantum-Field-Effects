            ...
            for g in couplings:
                for T in temperatures:
                    try:
                        ...
                        assert False, f"Failed for params m={m}, g={g}, T={T}: {e}"
        
        # Should have results for all parameter combinations
        assert len(results_matrix) == len(field_masses) * len(couplings) * len(temperatures)
    ...
    def test_physics_consistency_across_scales(self):
        ...
        for i, params in enumerate(scales):
            ...
            for t in times:
                ...
                assert np.isreal(A), f"Non-real amplification at scale {i}, time {t}"
                assert A > 0, f"Non-positive amplification at scale {i}, time {t}"
                assert A < 10, f"Unphysically large amplification at scale {i}, time {t}"
            ...
            S = results['S_mean']
            assert S <= 2 * np.sqrt(2) + 1e-10, f"Tsirelson violation at scale {i}: S={S}"
